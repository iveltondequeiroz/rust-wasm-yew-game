        ...
        let move_button = |target: &u8| {
            ...
            html! {
                <div class="control-button",>
                    <button onclick=|_| Msg::ButtonPressed(SwitchRoom(t)),>{&format!("Mover para  {}", target)}</button>
                    <button onclick=|_| Msg::ButtonPressed(ShootArrow(t)),>{&format!("Flechar {}", target)}</button>
                </div>
            }
        };
        html! {
